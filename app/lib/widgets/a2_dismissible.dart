...
/// Signature used by [Dismissible] to indicate that the dismissible has been
/// dragged.
///
/// Used by [Dismissible.onUpdate].
typedef DismissUpdateCallback = void Function(WDismissUpdateDetails details);
...
/// {@end-tool}
///
/// Backgrounds can be used to implement the "leave-behind" idiom. If a
/// background
/// is specified it is stacked behind the Dismissible's child and is exposed
/// when
/// the child moves.
///
...
class A2Dismissible extends StatefulWidget {
  ...
  /// has been dragged up or to the left. It may only be specified when
  /// background
  ...
  /// Called when the widget changes size (i.e., when contracting before being
  /// dismissed).
  ...
  /// dismissible will begin at the position where the drag gesture won the
  /// arena.
  ...
  @override
  State<A2Dismissible> createState() => _A2DismissibleState();
}
...
class WDismissUpdateDetails {
  /// Create a new instance of [DismissUpdateDetails].
  WDismissUpdateDetails({
    this.direction = DismissDirection.horizontal,
    this.reached = false,
  ...
  /// invoked.
  ///
  /// This can be used in conjunction with [DismissUpdateDetails.reached] to
  /// catch the moment
  ...
  /// doing on screen,
  /// e.g. using this value to set the opacity thereby fading dismissible as
  /// it's dragged offscreen.
...
class _A2DismissibleState extends State<A2Dismissible>
  ...
  void _handleDragUpdate(DragUpdateDetails details) {
    ...
    if (!_moveController!.isAnimating) {
      ...
      _dragProgress.value = _dragExtent.abs() / _overallDragAxisExtent;
    }
  }
  ...
  _FlingGestureKind _describeFlingGesture(Velocity velocity) {
    ...
    // Verify that the fling is in the generally right direction and fast
    // enough.
    if (_directionIsXAxis) {
  ...
  Widget build(BuildContext context) {
          ...
          if (_resizeAnimation!.status != AnimationStatus.forward) {
            ...
            throw FlutterError.fromParts(<DiagnosticsNode>[
              ...
              ErrorHint(
                ...
                'immediately remove the Dismissible '
    ...
    // If the DismissDirection is none, we do not add drag gestures because the
    // content
    // cannot be dragged.
